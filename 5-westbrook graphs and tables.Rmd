...
ggRawJimmy <- 
ggplot(tempDataSync[tempDataSync$riverOrdered == 'WB JIMMY' & tempDataSync$year>2006,],aes(dOY,temp))+
  geom_line(size=0.5)+
...
ggsave( file=paste0(graphsDir,'ggRawJimmy.png'), plot=ggRawJimmy, dpi=600 , width=6,height=5, units='in', scale=2 )



ggplot(tempDataSyncS, aes(dOYInt,temp)) + geom_boxplot(aes(fill=factor(dOYInt))) + facet_wrap(~riverOrdered)
...
ggplot( r[r$incompleteYear & !is.na(r$riverOrdered),], aes(incompleteIndex,diff)) + geom_point()

ggRMSEDOY <- 
r %>%
  filter( incompleteYear ) %>%
  group_by( scenario ) %>%
...
ggplot( . ,aes(dOYDiff,mean,shape=factor(s))) +
  geom_point( size=4 ) + 
  geom_smooth(method='lm'
              #, formula=y~x+I(x^2)
              ,se=F,aes(linetype=factor(s)),color='black') +
