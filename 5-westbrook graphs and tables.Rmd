...
meanTrainData <- pS1 %>%
  ...
  ungroup() %>%
 # select(scenario, rivsIndex, year, rmseTrain)
 select(scenario, rivsIndex, rmseTrain)


#pS <- left_join(pS1,meanTrainData, by=c("scenario", "rivsIndex", "year")) %>%
pS <- left_join(pS1,meanTrainData, by=c("scenario", "rivsIndex")) %>%
  mutate( diff = rmseByKeep - rmseTrain )
