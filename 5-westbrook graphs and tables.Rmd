...
ggV <- list()
for(r in unique(tempDataSyncS$riverOrdered)){
  ggV[[r]] <- 
    
   ggplot(tempDataSyncS[tempDataSyncS$riverOrdered==r & tempDataSyncS$nMonthRiverYear >= 20 & tempDataSyncS$year >=2003,], aes((month),temp)) + 
    geom_violin() + 
    geom_point(aes((month),meanByMonth),shape=16,size=2) +
    geom_point(aes((month),meanByMonthRiver),shape=1,size=3) +
    geom_point(aes((month),meanByMonthRiverYear),shape=95,size=5) +
...
ggViolin <- arrangeGrob(ggV[[1]],ggV[[2]],ggV[[3]],ggV[[4]], ncol=2)#, widths=c(5,1.5))  
...
g <- data.frame(s=tempDataSyncS$site,y=tempDataSyncS$year,dOY=tempDataSyncS$dOY,t=tempDataSyncS$temp,r=gGam$residuals,f=gGam$fitted.values)
...
ggplot(gSum, aes(dOY,cR,color=factor(y))) + geom_point() + geom_hline(yintercept=0) + facet_wrap(~s)
ggplot(gSum, aes(dOY,delta, color=factor(y))) + geom_line() + geom_hline(yintercept=0) + facet_wrap(~s)


for(yy in sort(unique(gSum$y))){
gg <-   
ggplot(filter(gSum,y==yy), aes(dOY,t)) + geom_point() + geom_line() +
  geom_line(aes(dOY,f),color='red')+
  geom_hline(yintercept=0) + facet_wrap(~s) +
  ggtitle(yy)
print(gg)
...
print(gg2)
  ...
  ggplot(filter(gSum,y==yy), aes(dOY,delta)) + geom_line(color='red') + geom_line  (aes(dOY,r)) + geom_hline(yintercept=0) + facet_grid(~s)+ggtitle(yy)
...
ggCubic <- 
  ggplot(pC$predCubic, aes(dOY,pred)) + geom_line(aes(color=factor(year)),size=1.25) +
...
keep <- keep[,c(1,3)] # cant get this to work in dplyr. dplyr named the first 2 cols'scenario' 
...
pS <- predictSummary %>%
  ...
  group_by( scenario,rivsIndex,year,outYear,inKeep ) %>%
  summarise( residByKeep = rmse( resid ) )

ggplot(pS, aes(inKeep,residByKeep, color=factor(rivsIndex))) +
  geom_jitter() +
 # geom_line() +
  facet_wrap(~scenario)

ggplot(pS, aes(residByKeep,color=factor(inKeep)))+
  geom_freqpoly() +
  facet_grid(scenario~rivsIndex+outYear)
```  
