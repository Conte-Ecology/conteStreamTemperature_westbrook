...
#setwd('D:/projects/temperatureProject/')
...
fileOutName <- paste0(dataOutDir,'outP.RData')
bugsName <- paste0(baseDir,"code/correlatedSlopes.txt")

# source(paste0(baseDir, 'code/functions/temperatureModelingFunctions.R')) # now in streamTemperature package

load(paste0(dataInDir, 'MAUSGS/etSWB.RData'))
```
...
cat("
    ...
    for(i in 1:n){ # n observations
      ...
      stream.mu[i] <- inprod(B.0[], X.0[i, ]) + inprod(B.site[site[i], ], X.site[i, ]) + B.year[year[i]] #  
    }
    ...
    for(t in 1:Ti){
    B.year[t] ~ dnorm(0, tau.B.year)
    }
    
    sigma.b.year ~ dunif(0, 100)
    tau.B.year <- 1 / (sigma.b.year * sigma.b.year)
  }
    ", fill = TRUE)
sink()
```
...
variables.fixed <- c("intercept", "flow", "dOY", "dOY2", "dOY3")
...
nIterChunk <- 500 #round(nIter/nSave)   # num of iters per chunk
nSave <- round(nIter/nIterChunk)                       # num of saves

nChainsCL <- 3
cl <- makeCluster(nChainsCL)
clusterExport(cl=cl, list("data", "inits", "params", "K", "J", "Ti", "L", "n", "W.site", "W.year", "X.site", "X.year", "nChains","nAdapt", "nIterChunk", "nThin",'bugsName'), envir = environment())
clusterSetRNGStream(cl=cl, iseed = 2345642)

sampleJagsInChunks(cl,fileOutName,bugsName,data,inits,nAdapt,params,nIterChunk,nThin)
load(fileOutName)  #loads 'outP'                         
out <- appendJagsChunks( outP,nThin,nIter,nIterChunk )
...
temp2 <- melt(((out$deviance[,,])))
temp2 <- melt(((out$sigma[,,])))
...
temp2 <- melt(((out$B.0[,,])))
temp2 <- melt(((out$mu.year[,,])))
temp2 <- melt(((out$mu.site[,,])))
temp2 <- melt(((out$sigma.b.year[,,])))
temp2 <- melt(((out$B.year[,,])))
