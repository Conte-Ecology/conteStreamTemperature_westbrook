...
system.time(M.wb <- modelRegionalTempWB(tempDataSyncS, firstObsRows, evalRows, data.fixed=data.fixed, data.random.years=data.random.years, n.burn = 100, n.it = 100, n.thin = 2, nc = 3, coda = coda.tf, param.list = monitor.params))
...
```{r striaght up cross-validation - leave out 30% randomly across years}
...
train <- c(rep(F,10),rep(T,10)) 

# loop over propOut
propOut <- seq(0,0.5,0.1)
for (p in propOut)
 ...
 for ( i in 1:20 ){
  ...
  if (!train[i]) dat$temp <- ifelse( dat$keep, dat$temp, NA )  #set 30% of temp values to NA for non-training runs
  ...
  rmseByOutAllCV <- rbind(rmseByOutAllCV,cbind(propOut=p,train=train[i],rmseAll=rmse(tempDataSyncS$resid),rmseByKeep))

  } # rep bracket 
} #proportion of data bracket

summ <- rmseByOutAllCV %>% group_by( p,train ) %>% summarise( mean=mean( rmse ), sd=sd( rmse ), 
                                                            loCI= quantile(rmse,probs=0.025,na.rm=T),hiCI= quantile(rmse,probs=0.0975,na.rm=T) )


#predictSummary <- na.omit(predictSummary)

save( rmseByOutAllCV,summ, file = paste0(dataLocalDir,'crossValidation30.RData'))
