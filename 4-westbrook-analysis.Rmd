...
```{r cross-validation - 5 scenarios}
...
predictSummary <- ggObsPred <- list()
rm(rmseByOut,rmseByOutAll)
rmseByOut <- rmseByOutAll <- NA

# scenarios to test. set up  different sets of days to keep
nDays <- 30 # num of days to keep

loDOY  <- quantile(tempDataSyncS$dOYInt,0.025)
mDOY  <- quantile(tempDataSyncS$dOYInt,0.5)
hiDOY  <- quantile(tempDataSyncS$dOYInt,0.975)

rm(dOYKeep1); dOYKeep1 <- matrix(NA,26,nDays*10)
...
for ( i in 1:( nrow(years) + 0 ) ){
  ...
  #Don't do any estimation if there are no evalRows for this year
  if( length(intersect( dat[ dat$year == y,'rowNum' ],evalRows ) ) > 0 ){
  ...
  predictSummary[[inc]] <- data.frame( scenario = k, yearOut = y, pred = tempDataSyncS$pred.wb, resid = tempDataSyncS$resid )
  ...
  ggplot(tempDataSyncS
    ...
    ggtitle(paste('Scenario = ',k,' Year =',y)) +
    facet_wrap(~riverOrdered)
...
save( rmseByOut,rmseByOutAll,predictSummary,ggObsPred, file = paste0(dataLocalDir,'validation.RData'))
...
# Look at deviance from base case
