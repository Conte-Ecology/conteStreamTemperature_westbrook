...
system.time(M.wb <- modelRegionalTempWB(tempDataSyncS, data.fixed=data.fixed, data.random.years=data.random.years, n.burn = 5000, n.it = 1000, n.thin = 5, nc = 3, coda = coda.tf, param.list = monitor.params))
...
y <- data.frame(year=unique(tempDataSyncS$year))
years <- arrange(y,year)
...
for ( i in 1:nrow(years) ){
  ...
  dat <- tempDataSyncS
  dat$temp[ dat$year %in% y ] <- NA
  
  system.time(M.wb <- modelRegionalTempWB(dat,
                                          data.fixed=data.fixed, 
                                          data.random.years=data.random.years, 
                                          n.burn = 500, n.it = 1000, n.thin = 5, nc = 3,
