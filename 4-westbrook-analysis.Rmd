...
evalRows <- createEvalRows(dat)
  ...
  for(ii in 1:length(tempDataSyncS$temp)) {
    ...
    tempDataSyncS$resid.wb[ii] <- mean(huc.mat[ , paste0("residuals[", ii, "]")])
    tempDataSyncS$pred.wb[ii] <- mean(huc.mat[ , paste0("stream.mu[", ii, "]")])
  }
...
save( rmseByOut,cVSummary,predictSummary, file = paste0(dataLocalDir,'validation.RData'))
...
ggplot(tempDataSyncS[
 ...
 #  geom_line(aes(dOY,pred,color=factor(year)),data=predCubic)+
  geom_point(color='red') +
#  geom_point( aes(dOY,0), color='green',data=dat[dat$year %in% yearPlot & !is.na(dat$temp),] ) +
  geom_point( aes(dOY,0), color='darkgreen',data=dat[dat$year %in% yearPlot & 
                                                     dat$rowNum %in% evalRows$rowNum,]  ) +
  facet_grid(year~riverOrdered)
